...
- Each OAuth grant gets its own device id, minted at `/oauth/authorize` and sent to Auth0 as a `device:<id>` scope, so the Auth0 action keys MFA presence per MCP client installation rather than per user (ADR 0054); a client that sends a usable `device:` scope of its own keeps it. The id rides back to the client sealed inside the authorization code and the refresh token, both opaque values the client echoes unchanged; `/oauth/token` unseals them, exchanges the bare Auth0 value, and seals the rotated refresh token again. A refresh carries no scope, so there the id is also forwarded as `device_id`; a code exchange needs none, since the `device:` scope named it at login. The access token is Auth0's and is never touched. The seal is an HMAC under a key derived from `AUTH0_CLIENT_SECRET` by default; the optional `ALPACON_MCP_GRANT_SECRET` sets an explicit one under the same hex and 32-byte rules as the state key. Rotating either invalidates every sealed refresh token, so every remote session logs in again; that is why the key is separate from `ALPACON_MCP_STATE_SECRET`, which only has to outlive a ten-minute login
- `/oauth/token` exchanges only values this server sealed: a code or refresh token without a valid seal, including a refresh token issued before sealing was deployed, is answered with `invalid_grant` and never reaches Auth0. Deploying this therefore logs every remote session in once more
