...
- **Every other `*_id` is checked before the request is built.** These identifiers are interpolated into the endpoint path, so a value must be a string of letters, digits, `.`, `_`, `~`, or `-`, and must not be `.` or `..`. Anything else—a `/`, a `?`, a `#`, a percent-encoded spelling of one, or a value that is not a string at all—comes back as a validation error naming the field, and no request is sent. `work_session_id` is exempt: it travels in the request body rather than the path, and surrounding whitespace is stripped.
