...
_HINT_REGISTRY: dict[tuple[int, str], dict[str, list[str]]] = {
    ...
    (HTTPStatus.NOT_FOUND, 'alert'): {
        'recovery_hints': [
            ...
            (
                'A 404 on attach_alert_rule or detach_alert_rule means a wrong '
                'server_id; a bad alert rule ID there gives a 400 instead.'
            ),
            (
                'A 404 on preview_alert_rule means no rule with that rule_id, or '
                'every server the rule is attached to is outside what you can see.'
            ),
        ],
