...
1. **Create Tool File**
   ...
   **`**kwargs` is mandatory**: it is where the decorator injects the token, and a tool that omits it fails decoration with a `TypeError` at import time. It never reaches the published input schema—the decorator strips the catch-all from the signature FastMCP reads, because FastMCP would publish it as a required string field. For the same reason, never forward your own `**kwargs` on to another decorated tool: by then the catch-all holds the resolved credential, and the receiving tool does not accept it as a parameter. `TestCatchAllForwarding` fails the build on the direct form.
