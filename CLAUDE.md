...
- **`@mcp_tool_handler` owns validation, token injection, error shape, and
  ...
  guard. `TestCatchAllForwarding` pins the direct form—a call handing its own
  `**kwargs` straight to another tool—at any log level; an indirect forward,
  such as `**{**kwargs}` or a single `token=kwargs.get('token')`, is still
  caught by the bind alone and so only at INFO. `health_check` sits outside the
  decorator entirely because it must answer before any JWT exists, and borrows
  `@with_error_handling` alone for the error shape.
